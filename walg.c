...
walscandir(Wal *w)
{
    ...
    closedir(d);
    //上面的while循环会检索当前目录下的binlog.1，binlog.2，binlog.3的文件
    //计算出最大的.X
    //然后将全局的Wal的next值变成X+1，这也就是为啥每次beanstalkd启动的时候会创建一个更大的
    //binlog.X+1的原因
    w->next = max + 1;
    return min;
}
...
walinit(Wal *w, job list)
{
    ...
    // first writable file
    // 用walscandir里计算出来的那个next值去创建文件，next的值是上一次扫描到的最大的index+1
    // 所以新文件一定会被创建出来
    if (!makenextfile(w)) {
        twarnx("makenextfile");
        exit(1);
