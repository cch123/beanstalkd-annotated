...
prot_init()
{
    ...
    //这里读取的是linux的随机数文件，另外一个可用的文件是
    //dev/random，这个文件在读取时会阻塞程序，所以速度慢一些，一般都用urandom
    ...
    //转为十六进制数，两个字节两个字节地写入到全局的id数组里
    //这个id看起来就只是为了给beanstalkd的实例一个唯一的id值而已
    //每次启动都是不一样的(废话)
    //static char id[NumIdBytes * 2 + 1]; // hex-encoded len of NumIdBytes
    ...
    //初始化全局tubes，传两个空的回调函数
    ms_init(&tubes, NULL, NULL);

    //初始化全局的default的tube
    //这个也是beanstalkd在你什么都不做的时候默认就会有的一个tube
    //这段宏有点看不懂，需要再看一下
